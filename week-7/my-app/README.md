...
Страница собирается при каждом запросе, данные о книге запрашиваются у API в реальном времени. можно использовать `headers`, `cookies`, `user-agent`, `nonce`. Он работает медленнее, т.к. ждём ответ от API и нагрузка растёт с ростом числа пользователей

## SSG/ISR (getStaticProps/revalidate)

Страница генерируется один раз при билде, а потом переcобирается через определенный интервал, такое не подходит для часто обновляющиеся данные, нет доступа к заголовкам
